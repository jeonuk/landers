$(document).ready(function(){
    ...
    // 윈도우의 가로길이가 1280 미만이면 모바일 버전 메뉴가 나타남
    if($(window).width() >=1280){
      ...
      $('header').mouseleave(function(){  
        ...
        $('nav > ul > li > a').click(function(){
            //a태그의 다음 객체인 ul.sub가 slideToggle 됨 
            $(this).next().slideToggle();
            //클릭하지 않은 a객체의 ul.sub는 slideUp 됨
            $('nav > ul > li > a').not(this).next().slideUp();
            // 클릭한 주메뉴의 active클래스를 추가하거나 제거.
            $(this).toggleClass('active');
            // 클릭하지 않은 주메뉴는 active 제거
            $('nav > ul > li > a').not(this).removeClass('active');
            return false;
        });
        // header의 메뉴버튼을 클릭하면 내비게이션 나타나고, 다시 클릭하면 들어감
        let menu_sw=0;
  ...
  minimize_header();
    ...
    // 스크롤 내릴 대마다 section 안의 자식 콘텐츠들이 서서히 나타나는 애니메이션
   $(window).scroll(function(){
    ...
    $('.scrollSet').each(function(){
      ...
      // 만약 secTop이 winTop 보다 작고, secBottom이 winTop보다 크면 
      // section의 자식객체 active 추가
